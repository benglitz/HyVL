...
% DivC = axesDivide(DivX,DivY,X0,Y0,SepXs,SepYs)
% DivC = axesDivide(DivX,DivY,Pos0,SepXs,SepYs)
%
...
switch lower(Option)
  case {'c','create'}; 
    for i=numel(DivC):-1:1 AH(i) = axes('Pos',DivC{i}); end 
    AH = reshape(AH,size(DivC));
  case ''; 
